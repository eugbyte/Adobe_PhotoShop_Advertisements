...
function generateBalance(){
    var bootstrapDiv = document.createElement('div');
    bootstrapDiv.className = 'col-sm-12 bootstrapDiv balanceTable';
    document.body.appendChild(bootstrapDiv);
...
function editRow(event){
    ...
    //remove the key value pair from the income/expenses object
    if (table.id == 0){
        income.removeItems(item);
    } else if (table.id == 1){
        expenses.removeItems(item);
    }

    //Update the balance table
        //delete the balance table
    var balanceTable = document.querySelector('.balanceTable');
    balanceTable.remove();
        //regenerate the balance table
    generateBalance();
